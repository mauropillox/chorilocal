...
# Backup, validate, build, deploy, and verify
...
if [[ "${SKIP_PULL:-}" != "1" ]]; then
    ...
    git pull || log "   (git pull skipped or failed)"
else
...
if ! grep -q "VITE_API_URL=http" frontend/.env; then
    log "   ⚠️ VITE_API_URL not properly defined in frontend/.env"
    exit 1
fi
...
if docker compose ps backend --format json 2>/dev/null | grep -q running; then
    ...
    docker cp chorizaurio-backend:/tmp/ventas.db.bak "$BACKUP_DIR/ventas.db.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null && \
        log "   ✓ Database backed up" || log "   (DB backup skipped)"
else
    # Backup local if container not running
