...
from pydantic import BaseModel, Field, field_validator          # ← Field agregado
...
# --- Asegurar base de datos persistente ---
...
# --- Crear/verificar tablas al iniciar ---
db.crear_tablas()
...
def login(username: str = Form(...), password: str = Form(...)):
    ...
    token = create_access_token({"sub": str(user["id"]), "rol": user["rol"], "activo": user["activo"]})
...
def update_producto_endpoint(producto_id: int, producto: Producto, user=Depends(get_current_user)):
    return db.update_producto(producto_id, producto.dict(exclude_none=True))
...
def get_pedidos_pendientes(usuario: dict = Depends(get_current_user)):
    filtro = "(pdf_generado IS NULL OR pdf_generado = 0) AND (pdf_descargado IS NULL OR pdf_descargado = 0)"
    pedidos = (
        db.get_pedidos_filtrados(filtro)
        if usuario["rol"] == "admin"
        else db.get_pedidos_filtrados(filtro, user_id=usuario["id"])
    )
    return enriquecer_pedidos(pedidos)
...
def get_pedidos_generados(usuario: dict = Depends(get_current_user)):
    filtro = "pdf_generado = 1 AND (pdf_descargado IS NULL OR pdf_descargado = 0)"
    pedidos = (
        db.get_pedidos_filtrados(filtro)
        if usuario["rol"] == "admin"
        else db.get_pedidos_filtrados(filtro, user_id=usuario["id"])
