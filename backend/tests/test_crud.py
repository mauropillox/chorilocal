...
class TestClientes:
    ...
    def test_get_cliente_by_id(self, client, auth_headers):
        ...
        # Create first
        create_response = client.post("/clientes", headers=auth_headers, json={
        ...
        # Get by ID
        response = client.get(f"/clientes/{cliente_id}", headers=auth_headers)
        assert response.status_code == 200
    ...
    def test_update_cliente(self, client, auth_headers):
        ...
        # Update
        response = client.put(f"/clientes/{cliente_id}", headers=auth_headers, json={
        ...
        if "nombre" in data:
            ...
            # Verify by fetching
            get_response = client.get(f"/clientes/{cliente_id}", headers=auth_headers)
            assert get_response.json()["nombre"] == "Cliente Actualizado"
    
    def test_delete_cliente_requires_admin(self, client, user_headers):
        """Delete requires admin role"""
        response = client.delete("/clientes/1", headers=user_headers)
...
class TestProductos:
    ...
    def test_get_productos(self, client, auth_headers):
        """Get all products"""
        # Create first
        client.post("/productos", headers=auth_headers, json={
            "nombre": "Producto 1",
            "precio": 50.0
        })
        client.post("/productos", headers=auth_headers, json={
    ...
    def test_search_productos(self, client, auth_headers):
        ...
        response = client.get("/productos?q=morcilla", headers=auth_headers)
        assert response.status_code == 200
        productos = response.json()
    ...
    def test_update_producto(self, client, auth_headers):
        """Update a product"""
        create_response = client.post("/productos", headers=auth_headers, json={
            "nombre": "Producto Original",
            "precio": 100.0
        })
        producto_id = create_response.json()["id"]
        
        response = client.put(f"/productos/{producto_id}", headers=auth_headers, json={
...
class TestCategorias:
    ...
    def test_create_categoria(self, client, auth_headers):
        ...
        response = client.post("/categorias", headers=auth_headers, json={
            "nombre": "Embutidos",
            "color": "#FF5733"
        })
        assert response.status_code == 200
    ...
    def test_get_categorias(self, client, auth_headers):
        """Get all categories"""
        response = client.get("/categorias", headers=auth_headers)
        assert response.status_code == 200
